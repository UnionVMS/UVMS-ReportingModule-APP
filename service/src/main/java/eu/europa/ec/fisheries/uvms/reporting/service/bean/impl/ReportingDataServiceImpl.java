...
import static eu.europa.ec.fisheries.uvms.reporting.service.entities.FilterType.fa;
...
//        long limit = pageSize != null ? pageSize : 1000;
        ...
        StringBuilder query =
                new StringBuilder()
                        .append("SELECT DISTINCT a.id,a.act_id,a.fa_report_id,a.trip_id,a.report_type,a.activity_type,a.purpose_code,a.source,a.accepted_timestamp,a.calculated_timestamp," +
                                ...
                                "t.number_of_corrections,t.number_of_positions,t.asset_hist_guid AS trip_assetHistGuid, al.type, al.type_code, ag.gear_code, acl.catch_location_type, acl.catch_location_code, ac.species_code ")
        ...
        if(!filterFilters(report).isEmpty()){
            query.append("WHERE    ");
                ...
                switch (f.getType()) {
                    case fa:
                        ...
                        if(faFilter.getFaPorts() != null && !faFilter.getFaPorts().isEmpty()) {
                            query.append(" (");
                            String collectPorts = faFilter.getFaPorts().stream().map(t -> " al.code like '%" + t + "%'" ).collect(Collectors.joining(" OR "));
                            query.append(collectPorts);
                            query.append(") AND");
                        }

                        if(faFilter.getMasters() != null && !faFilter.getMasters().isEmpty()) {
                            query.append(" (");
                            String collectMasters = faFilter.getMasters().stream().map(t -> " a.master like '%" + t + "%'" ).collect(Collectors.joining(" OR "));
                            query.append(collectMasters);
                            query.append(") AND ");
                        }

                        if(faFilter.getSpecies() != null && !faFilter.getSpecies().isEmpty()){
                            query.append(" (");
                            String collectSpecies = faFilter.getSpecies().stream().map(t -> " ac.species_code = '" + t + "'" ).collect(Collectors.joining(" OR "));
                            query.append(collectSpecies);
                            query.append(") AND ");
                        }
        ...
        prepareAssetSubQuery(query,report);
...
//        nativeQuery.setMaxResults(maxResults);
    ...
    private Set<Filter> filterFilters(Report report){
        return report.getFilters().stream().filter(l -> vgroup.equals(l.getType()) || asset.equals(l.getType()) || fa.equals(l.getType())).collect(Collectors.toSet());
    }

    private void prepareAssetSubQuery(StringBuilder query,final Report report){
        List<String> assetGuids = findAssetGuids(report);

        if (assetGuids.isEmpty()){
            return ;
        }
        if(assetGuids.size() == 1 ){
            query.append("(ra.asset_hist_guid = '" + assetGuids.get(0) +"') AND ");
            return ;
        }
        query.append(" (");
        String collectedGuids = assetGuids.stream().map(t -> "ra.asset_hist_guid = '" + t + "' ").collect(Collectors.joining(" OR "));
        query.append(collectedGuids);
        query.append(") AND ");
    }

    private List<String> findAssetGuids(final Report report){
        List<String> assetGuidList = new ArrayList<>();
        for(Filter filter :report.getFilters()){
            if(asset.equals(filter.getType())){
                assetGuidList.add(((AssetFilter)filter).getGuid());
            }
        }
